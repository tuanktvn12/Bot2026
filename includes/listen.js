  ...
  setInterval(async() => {
    ...
    if (day != day_now) {
      ...
      checkttData.forEach(async(checkttFile) => {
        ...
        let checkttBody = '';
                    checkttBody += storage.slice(0, 0).map(item => {
                        return ``;
      }).join('\n');
...
//const id = "100000272507589"
 ...
 return async(event) => {
    ...
    if (event.type == "change_thread_image") api.sendMessage(`Eiros - ${event.snippet}`, event.threadID);
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`=====『 𝗥𝗘𝗤𝗨𝗘𝗦𝗧 』=====\n→Box ${threadName} của bạn chưa được duyệt\n→TID: ${event.threadID}\n→Thời gian: ${timeVN}\n→Để gửi yêu cầu duyệt\n→Hãy dùng: ${prefix}request\n→Hoặc ib với link FB\n→Fb.me/779743414\n→Hãy ib để yêu cầu được duyệt nhé`, event.threadID);
